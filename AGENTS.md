...
Use `VIDEO_MODE=1 bun run spec --grep "my test name"` to enable the copied video-mode plugin, which highlights locator actions and pauses around them so Playwright videos are easier to review. Before uploading Playwright videos to a PR, run `bun spec/plugins/video-mode.ts trim <video.webm|test-results-directory>` so leading blank white frames are removed and GitHub's inline preview starts on the first real app frame.
