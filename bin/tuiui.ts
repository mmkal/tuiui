...
import { z } from "zod";
import { runCodexTmux } from "../src/codex-tmux.ts";

export const router = t.router({
  codex: t.procedure
    .meta({
      description: "Run Codex in tmux, attaching resume commands to matching live tmux sessions.",
      examples: [
        "tuiui codex",
        "tuiui codex --yolo",
    ...
    .handler(async ({ input }) => {
      process.exitCode = await runCodexTmux(input || []);
    }),
});
