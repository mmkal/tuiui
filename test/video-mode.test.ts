...
test("parses ffmpeg signalstats frame metadata", () => {
  ...
  expect(frames).toEqual([
    { time: 0, yMin: 235, yAvg: 235, satAvg: 0 },
    { time: 0.1, yMin: 29, yAvg: 29, satAvg: 56 },
  ]);
...
testWithFfmpeg("trims leading white frames from a Playwright-style WebM", async () => {
  ...
  try {
    ...
    writeWhiteThenNavyWebm(videoPath);
...
function writeWhiteThenNavyWebm(videoPath: string) {
  execFileSync("ffmpeg", [
    "-hide_banner",
    "-loglevel",
    "error",
    ...
    "lavfi",
    "-i",
    "color=navy:size=160x90:rate=30:d=0.7",
    "-filter_complex",
