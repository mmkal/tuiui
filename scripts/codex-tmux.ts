...
async function main() {
  ...
  if (args[0] === WATCH_ARG) {
    await watchSessionId(args[1]);
    return;
  }

  const realCodex = findRealCodex();
  if (!realCodex) {
    console.error("codex-tmux: could not find the real codex binary. Set CODEX_TMUX_CODEX_BIN.");
    process.exitCode = 127;
    return;
  }

  if (process.env.CODEX_TMUX === "0" || !hasCommand("tmux")) {
    process.exitCode = runForeground(realCodex, args);
    return;
  }
  ...
  if (resumeId) {
    ...
    if (existingSession) {
      process.exitCode = attachTmux(existingSession);
      return;
    }
  }
  ...
  if (process.env.TMUX) {
    ...
    process.exitCode = attachTmux(tmuxSession);
    return;
  }
...
function findRealCodex() {
  const configured = process.env.CODEX_TMUX_CODEX_BIN;
  if (configured) {
    return configured;
  }
...
function startWatcher(sessionName: string, command: string) {
  const child = spawn(process.execPath, [new URL(import.meta.url).pathname, WATCH_ARG, sessionName], {
    detached: true,
    stdio: "ignore",
    env: {
      ...process.env,
...
async function watchSessionId(sessionName: string | undefined) {
  if (!sessionName) {
    process.exitCode = 2;
    return;
  }
